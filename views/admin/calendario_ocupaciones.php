...
<body>
    ...
    <?php if($msg): ?>
        <div class="mensaje-alert mensaje-exito" id="mensajeAlert" style="max-width: 1600px; margin: 20px auto; padding: 0 40px;">
            <div style="background: #d1fae5; color: #065f46; border: 1px solid #6ee7b7; padding: 16px 24px; border-radius: 12px; font-size: 15px; font-weight: 600;">
                ...
                if($msg == 'carga_exitosa') {
                    $creados  = intval($_GET['creados']  ?? 0);
                    $fallidos = intval($_GET['fallidos'] ?? 0);
                    echo '<i class="bi bi-check-circle-fill"></i> Carga masiva completada: ' . $creados . ' ocupaciones registradas';
                    if($fallidos > 0) echo ' &nbsp;|&nbsp; <i class="bi bi-exclamation-triangle-fill"></i> ' . $fallidos . ' omitidos por conflicto o datos inválidos';
                }
                ?>
            </div>
        </div>
    ...
    <div class="encabezado">
        ...
        <div class="navegacion-mes">
            <a href="?sede_id=<?php echo $sede_id; ?>&mes=<?php echo $mes-1; ?>&anio=<?php echo $anio; ?>" class="btn-mes">
                ◀ Anterior
            </a>
    ...
    <script>
        ...
        function clickDia(dia, fecha, tieneOcupaciones) {
            ...
            if(esFechaPasada(fecha)) {
                if(tieneOcupaciones) {
                    ...
                    mostrarToast('No se pueden registrar ocupaciones en fechas pasadas');
                }
                return;
            }
        ...
        function irAFecha(fecha) {
            ...
            window.location.href = '?sede_id=<?php echo $sede_id; ?>&mes=' + parseInt(m) + '&anio=' + y + '&dia=' + parseInt(d);
        }
    ...
    <script>
    // ── Navegación semanal en móvil ──
    (function(){
        if(window.innerWidth > 768) return;

        let semana = <?php echo $semana_inicial_cal; ?>;
        const total = <?php echo $total_semanas_cal; ?>;
        const urlAnt = '?sede_id=<?php echo $sede_id; ?>&mes=<?php echo $mes_ant; ?>&anio=<?php echo $anio_ant; ?>&semana=last';
        const urlSig = '?sede_id=<?php echo $sede_id; ?>&mes=<?php echo $mes_sig; ?>&anio=<?php echo $anio_sig; ?>';
        ...
        function mostrar(n) {
            celdas.forEach(el => {
                el.style.display = (parseInt(el.dataset.week) === n) ? 'block' : 'none';
            });
            lbl.textContent = mesAnio + ' · Semana ' + (n + 1) + ' / ' + total;
        }

        document.getElementById('btnSemAnt').onclick = function() {
            if(semana > 0) { semana--; mostrar(semana); }
            else window.location.href = urlAnt;
        };
        document.getElementById('btnSemSig').onclick = function() {
            if(semana < total - 1) { semana++; mostrar(semana); }
            else window.location.href = urlSig;
        };
